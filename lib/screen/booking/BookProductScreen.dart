...
class BookProductScreenState extends State<BookProductScreen> {
  ...
  BookingProductBloc mBloc = BookingProductBloc(LoadingState());
  ...
  _selectTime(context, TypeEnum type) async {
...
// NO need to do anything
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: BlocProvider(
          create: (BuildContext context) => BookingProductBloc(LoadingState()),
  ...
  void showPreview() {
    ...
    if (durationType == DurationTypeEnum.HOURS.item) {
      ...
      selectedDateTo = selectedDateFrom.add(Duration(days: period * 30));
    }
    ...
    if (address.isEmpty) {
      Fluttertoast.showToast(msg: S
          .of(context)
          .addressShouldNotBeEmpty);
      return;
    }
    if (state.isEmpty) {
      Fluttertoast.showToast(msg: S
          .of(context)
          .stateShouldNotBeEmpty);
      return;
    }
    if (pinCode.isEmpty) {
      Fluttertoast.showToast(msg: S
          .of(context)
          .pincodeShouldNotBeEmpty);
      return;
    }
    ...
    if (widget.model.data.verificationRequired) {
      if (docId == null || docdl == null) {
        Fluttertoast.showToast(msg: S
            .of(context)
            .pleaseUploadDocument);
        return;
      }
    }
    Navigator.of(context).pushNamed("/preview",
        arguments: generateBody(
            fromDate,
            toDate,
            timeFrom,
            timeTo,
